...
from .serializers import (
    ...
    DistanceBasePriceSerializer,
    DistanceAdditionalPriceSerializer,
    TimeMultiplierFactorSerializer,
    UserSerializer,
)
...
class DistanceBasePriceListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if not pricing_config:
            return Response(
                {"error": "pricing config id mismatch"},
...
class CalculatePricingView(APIView):
    def post(self, request):
        try:
            ...
            if not pricing_config:
                return Response(
                    {"error": "No pricing configuration enabled"},
                    status=status.HTTP_400_BAD_REQUEST,
                )

            # Calculate distance-based price
            dbp = (
                DistanceBasePrice.objects.filter(
                    pricing_config=pricing_config, distance_in_km__lte=distance
                )
            ...
            if not dbp:
                return Response(
                    {"error": "Invalid distance"}, status=status.HTTP_400_BAD_REQUEST
                )

            # Calculate additional price
            dap = DistanceAdditionalPrice.objects.filter(
                pricing_config=pricing_config
            ).first()
            if not dap:
                return Response(
                    {"error": "No additional price found"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            tmf = (
                TimeMultiplierFactor.objects.filter(
                    pricing_config=pricing_config, time_in_minutes__gte=time_in_min
                )
                .order_by("time_in_minutes")
                .first()
            )
            if not tmf:
                return Response(
